...
def extract_text_from_pdf(pdf_path: str) -> str:
    ...
    if pdf_extraction_method == "pymupdf":
    ...
    elif pdf_extraction_method == "pdfminer":
        try:
    ...
    elif pdf_extraction_method == "pypdf2":
        try:
            text = ""
            with open(pdf_path, 'rb') as file:
                reader = pdf_extractor.PdfReader(file)
        ...
        return "Error: No PDF extraction library available. Install PyMuPDF, PDFMiner, or PyPDF2."
...
def extract_tables_from_pdf(pdf_path: str) -> List[Dict]:
    ...
    Returns:
        Список таблиц в виде структурированных данных
    """
    # Попытка извлечь таблицы с помощью библиотеки tabula-py (если установлена)
    try:
        import tabula
        logger.info("Using tabula-py for table extraction")
        
        tables = tabula.read_pdf(pdf_path, pages='all', multiple_tables=True)
        return [table.to_dict(orient='records') for table in tables]
    except ImportError:
        logger.warning("tabula-py not installed. Table extraction not available.")
        return []
    except Exception as e:
        logger.error(f"Error extracting tables with tabula-py: {str(e)}")
        return []

def find_financial_tables(text: str) -> Dict[str, str]:
    """
    ...
    Returns:
        Словарь с найденными разделами финансовой отчетности
    """
    sections = {}
...
def extract_key_metrics(text: str) -> Dict[str, Any]:
    ...
    Returns:
        Словарь с извлеченными метриками
    """
    metrics = {}
...
def analyze_financial_report(pdf_path: str) -> Dict:
    """
    ...
    financial_sections = find_financial_tables(text)
    ...
    # Составляем рекомендации на основе анализа
    recommendations = generate_recommendations(metrics, management_discussion, risk_factors, report_type)
...
def extract_related_keywords(section_name: str) -> List[str]:
    """
    ...
    Returns:
        Список связанных ключевых слов
    """
    ...
    return related_terms.get(section_name.lower(), ["financial", "balance", "income", "cash", "statement", "total", "net"])
...
def extract_specific_section(pdf_path: str, section_name: str) -> Dict:
    """
    Извлекает конкретный раздел из финансового отчета с улучшенным поиском и анализом.
    ...
    Returns:
        Словарь с информацией о разделе и найденными данными
    """
    try:
        ...
        section_patterns = {
            ...
            "key_metrics": [
                ...
                r"(?:financial|operating)\s+highlights",
        ...
        if not section_content:
            # Если не нашли по явным заголовкам, ищем в окрестности ключевых слов, но с ограничением
            related_words = extract_related_keywords(section_name)[:5]  # Берем только первые 5 ключевых слов
        ...
        # Извлекаем числовые данные с улучшенной точностью, но ограничиваем их количество
...
def generate_recommendations(metrics: Dict, management_discussion: str, risk_factors: str, report_type: str) -> List[Dict]:
    """
    Генерирует рекомендации на основе анализа финансовых данных.
    
    Args:
        metrics: Словарь с финансовыми метриками
        management_discussion: Текст раздела MD&A
        risk_factors: Текст раздела о факторах риска
        report_type: Тип отчета
        
    Returns:
        Список рекомендаций с обоснованиями
    """
    recommendations = []
    
    # Примечание: Это базовый анализ. Настоящий финансовый анализ должен быть намного сложнее.
    # В данной реализации мы делаем очень упрощенные выводы на основе ограниченных данных.
...
def summarize_report(analysis_result: Dict) -> str:
    """
    Создает текстовое резюме анализа отчета.
    
    Args:
        analysis_result: Результаты анализа отчета
        
    Returns:
        Текстовое резюме
    """
    if "error" in analysis_result:
        return f"Ошибка анализа отчета: {analysis_result['error']}"
    
    summary = []
    summary.append(f"# Анализ финансового отчета {analysis_result['company_name']}")
    summary.append(f"## Тип отчета: {analysis_result['report_type']}")
    summary.append(f"## Период: {analysis_result['period']}")
    summary.append("")
